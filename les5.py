...
Callback-функции и управление потоком данных
"""
import time
...
from scapy.all import IP, TCP, UDP, ICMP, ARP, Ether, sniff, AsyncSniffer
...
class NetworkSniffer:
    ...
    def start_sniffing(self, interface=None, filter_text="", packet_count=0) -> bool:
        ...
        try:
            ...
            print(f"Ошибка запуска сниффера {e}")
            return False
    ...
    def _process_packet(self, packet):
        ...
        # Уведомляем callback о новом пакете
        if self.new_packet_callback:
    ...
    def set_callbacks(self, packet_callback=None, error_callback=None):
        """Устанавливаем функции обратного вызова"""
        self.new_packet_callback = packet_callback
        self.error_callback = error_callback
        print("Callback-функции установлены!")
...
if __name__ == "__main__": # код ниже будет запущен только в случае запуска самого исходного файла (т.е. при импорте не будет запускаться)
    ...
    # Создаем свои callback-функции
    def my_packet_handler(packet_info):
        print(f"CALLBACK: {packet_info['protocol']} пакет от {packet_info['source']}")
    
    def my_error_handler(error_msg):
        print(f"CALLBACK ERROR: {error_msg}")
    
    # Устанавливаем callback-функции
    sniffer.set_callbacks(my_packet_handler, my_error_handler)
    
    # Захватываем 3 пакета
    print("Захватываем 3 пакета...")
    sniffer.start_sniffing(packet_count=100)
    sniffer.stop_sniffing()
    print(sniffer.packet_count)
